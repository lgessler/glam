...
(defn with-users
  [f]
  (let [uuid1 (user/create session {:name "ldg" :email "ldg@g.com" :password_hash "topsecret"})
        uuid2 (user/create session {:name "fdr" :email "fdr@g.com" :password_hash "othersecret"})]
...
(deftest create
  (testing "Users can be created and they return their ID"
    (let [uuid (user/create session {:name "amr" :email "emr@g.com" :password_hash "s3cr37"})
...
(deftest get-props
  (testing "All props returned"
    (is (= {:name "ldg" :email "ldg@g.com" :password_hash "topsecret"}
...
(deftest user-constraints
  (let [uuid-alice (user/create session {:name "alice" :email "alice@g.com" :password_hash "foo"})]
    (testing "Unique name"
      (is (thrown? ClientException (user/create session {:name "alice" :email "alice@g.com" :password_hash "bar"}))))

    (testing "Unique email"
      (user/set-email session {:uuid uuid-alice :email "alice@vidaliaonions.com"})
      (let [uuid-alice2 (user/create session {:name "alice2" :email "alice2@vidaliaonions.com" :password_hash "bar"})]
