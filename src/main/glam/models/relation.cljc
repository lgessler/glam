...
#?(:clj
   (pc/defmutation set-value [{:keys [node] :as env} {:relation/keys [id value] :as relation}]
     ...
     (cond
       (nil? (r/get node id))
       (server-error 404 (str "Relation with id " id " does not exist."))

       (not (string? value))
       (server-error 400 "Value must be a string.")

       (not (ma/ident-locked? env [:relation/id id]))
       (server-error 403 (ma/lock-holder-error-msg env [:relation/id id]))

       :else
       (if-let [result (r/merge node id {:relation/value value})]
         (server-message (str "Successfully set relation value to " value))
         (server-error 500 (str "Failed to modify relation " id))))))
...
#?(:clj
   (pc/defmutation set-source [{:keys [node] :as env} {:relation/keys [id source] :as relation}]
     ...
     (cond
       ...
       (server-error 400 "Source span does not exist")
...
#?(:clj
   (pc/defmutation create-relation
     ...
     (cond
       ...
       (server-error 400 "Relation layer does not exist")

       (not (:span/id (s/get node source)))
       (server-error 400 "Source span does not exist")

       (not (:span/id (s/get node target)))
       (server-error 400 "Target span does not exist")

       (not= (:span/layer (s/get node source)) (:span/layer (s/get node target)))
       (server-error 400 "Relations must be contained to a single span layer.")

       (not= (s/get-doc-id-of-span node source) (s/get-doc-id-of-span node target))
       (server-error 400 "Relations must be contained to a single document.")

       (not (ma/ident-locked? env [:span/id source]))
       (server-error 403 (ma/lock-holder-error-msg env [:span/id source]))
       ...
       (let [{:keys [success] new-id :id} (r/create node {:relation/value  value
         ...
         (if-not success
           (server-error 500 "Failed to create relation, please try again")
